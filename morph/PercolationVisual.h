...
namespace morph {
    ...
    //! The template argument Flt is the type of the data which this PercolationVisual
    //! will visualize.
    template <typename Flt>
    class PercolationVisual : public VisualDataModel<Flt>
    {
    public:
        PercolationVisual(GLuint sp, const Vector<float> _offset)
        {
            this->shaderprog = sp;
        ...
        void initializeVertices()
        {
            ...
            if (ndata > 0 && ncoords != ndata) {
                std::cout << "PercolationVisual Error: ncoords ("<<ncoords<<") != ndata ("<<ndata<<"), return (no model)." << std::endl;
                return;
            }
            if (nvdata > 0 && ncoords != nvdata) {
                std::cout << "PercolationVisual Error: ncoords ("<<ncoords<<") != nvdata ("<<nvdata<<"), return (no model)." << std::endl;
                return;
            }
